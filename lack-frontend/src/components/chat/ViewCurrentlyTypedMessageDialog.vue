<template>
  <q-dialog ref="dialogRef" @hide="onDialogHide">
    <q-card style="max-width: 1000px; width: 600px">
      <q-card-section>
        <div class="text-h6">
          <span class="text-bold">{{ user }}</span> is currently typing
        </div>
      </q-card-section>
      <q-separator />
  ...
  setup(props) {
    ...
    const message = computed(() => {
      if (
        ...
        store.state.chat.selectedChannel in
          store.state.chat.currentlyTypedMessages
      ) {
        return store.state.chat.currentlyTypedMessages[
          store.state.chat.selectedChannel
        ][props.user];
      }
      return '';
